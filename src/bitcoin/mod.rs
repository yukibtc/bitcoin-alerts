...
use self::constants::DEFAULT_RPC_TIMEOUT;
...
pub async fn run(
    ...
    loop {
        let blockchain_info = match rpc.get_blockchain_info(DEFAULT_RPC_TIMEOUT).await {
            Ok(data) => data,
            Err(e) => {
        ...
        let network_info = match rpc.get_network_info(DEFAULT_RPC_TIMEOUT).await {
            Ok(data) => data,
            Err(error) => {
